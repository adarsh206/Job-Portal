...
const MyJobs = () => {
    ...
    useEffect(() => {
        setIsLoading(true)
        fetch(`http://localhost:3000/myJobs/dsas@linear.com`)
        .then(res => res.json())
        .then(data => {
            setJobs(data);
            setIsLoading(false)
  ...
  return (
    <div className='mx-auto xl:px-24 px-4'>
        <div className='my-jobs-container'>
            <h1 className='text-center p-5 text-2xl'>All My Jobs</h1>
      ...
      <div className="flex flex-wrap items-center">
        ...
        <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
          <Link to={"/post-job"}><button className="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3
           py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Post A New Job</button>
          </Link>
        </div>
      </div>
    </div>
